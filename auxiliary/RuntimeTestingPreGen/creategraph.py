...
def extractAndGraph(fileName, ax, col, polyNum):
    ...
    if(polyNum == 1):
        a, b = np.polyfit(np.array(xVals), np.array(yVals), 1)
        return ax.plot(np.array(xVals), a * np.array(xVals) + b, color=col)[0]
    if(polyNum == 2):
...
fileStems = ["bash-multiple", "multiple", "single"]
lines = [extractAndGraph(f"{fileStems[0]}-rt.csv", ax1, "red", 1),
         extractAndGraph(f"{fileStems[1]}-rt.csv", ax1, "blue", 2),
         extractAndGraph(f"{fileStems[2]}-rt.csv", ax1, "green", 1)]
