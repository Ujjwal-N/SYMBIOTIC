...
contract Request {
    ...
    function checkValidatorMembership(address _validator)
        public
        view
        returns (bool)
    {
    ...
    function getRating(address _validator) public view returns (int8) {
        uint index = getValidatorIndex(_validator);
        require(
            index != numValidatorsAllowed,
            "The address is not part of the request"
        );
    ...
    function getMajorityOpinion() public view returns (int8) {
        ...
        for (uint i = 0; i < validators.length; i++) {
            repSum +=
                ratings[i].rating *
                network.getReputationScoreExcludingRequest(validators[i], this);
    ...
    function checkValidatorEligibility(address _potentialValidator)
        public
        view
        returns (bool)
    {
        bool isAThirdParty = (_potentialValidator != requestor) &&
            (_potentialValidator != device.getOwnerAddress()); //"Requestor or provider cannot be validator"
        return
            isAThirdParty &&
            (numValidatorsSignedUp < numValidatorsAllowed) &&
            !checkValidatorMembership(_potentialValidator);
    }
    ...
    function volunteerAsValidator() external {
        ...
        require(
            checkValidatorEligibility(msg.sender),
            "Validator is not eligible"
        );
        require(
            !network.isMalicious(msg.sender),
            "The validator is assumed to be malicious"
        ); //Network considers reputation scores of below 0 to be malicious
    ...
    function postValidatorDataKey(address _validator, bytes32 _dataKey)
        external
    {
        require(
            msg.sender == device.getDeviceAddress(),
            "Only the external account associated with the device can post the access key to the device's data"
        );
        require(
            checkValidatorMembership(_validator),
            "The validator is not part of the request"
        );
        uint index = getValidatorIndex(_validator);
        require(
            ratings[index].dataKey == empty,
            "The validator has already been given a data key"
        );
        ratings[index].dataKey = _dataKey;
        emit NewValidatorKeyPosted(
            _validator,
            _dataKey,
            device.getDeviceAddress(),
            device.getOwnerAddress()
    ...
    function postRequestorDataKey(bytes32 _dataKey) external {
        ...
        require(
            requestorDataKey == empty,
            "The requestor has already been given a data key"
        );
        require(
            getValidated(),
            "At least 3 validators must respond before a key can be posted"
        );
        ...
        network
            .getOwnerSC(device.getOwnerAddress())
            .updateDataRequestsFulfilled(this); //used for owner's reputation score calculation
    ...
    function postRating(bool _validData) external {
        ...
        require(
            checkValidatorMembership(msg.sender),
            "The validator is not part of the request"
        );
        ...
        require(
            ratings[index].dataKey != empty,
            "The device has not yet responded with the validator's key"
        );
        require(
            ratings[index].rating == 0,
            "The validator has already responded"
        );
        ...
        emit RequestValidated(
            getMajorityOpinion() != -1,
            numValidatorsResponded == numValidatorsAllowed,
            device.getDeviceAddress(),
            device.getOwnerAddress()
        );
